...
var ZKOffsetDirectory = require('./zkoffsetdirectory');
...
function Consumer(options) {
    ...
    var OffsetManager = null;
    if (options.offset_manager) {
        OffsetManager = options.offset_manager
    } else if (options.zookeeper) {
        OffsetManager = ZKOffsetDirectory
    } else {
        OffsetManager = OffsetDirectory
    }
...
Consumer.prototype.start = function() {
    ...
    .then(function(num_partitions) {
        ...
        if (num_partitions < self.num_workers) {
            throw new Error('only ' + num_partitions + 'partitions for ' + self.num_workers + ' workers');
        }
